...
function($scope,$state,DrugStoreService,CommandeService,
  currentCommand,produitList,clientList){
...
$scope.produitValueChanged=function(produit){
   ...
   console.log("produit value changed");
...
$scope.validateChange=function(){
    ...
    if( Number.isNaN(montantPayer) || ((montantPayer +reduction)> chiffreAffaire)){
      ...
      $scope.errorSubmited=false;
            ...
            CommandeService.update($scope.commande,function(response){
              if(response.status){
    ...
    var montantPayer=Number.parseInt($scope.commande.montantPayer);
     var reduction=Number.parseInt($scope.commande.reduction);
      ...
      if( !Number.isNaN(montantPayer) && ((montantPayer+reduction) <= chiffreAffaire)){
         ...
         $scope.resteAPayer=chiffreAffaire-montantPayer-reduction;
       }
};
